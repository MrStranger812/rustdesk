...
hbb_name = 'hamrahdesk' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/hamrahdesk.exe')
    os.chdir('../..')
    if os.path.exists('./hamrahdesk_portable.exe'):
        os.replace('./target/release/hamrahdesk-portable-packer.exe',
                   './rustdesk_portable.exe')
    else:
        os.rename('./target/release/hamrahdesk-portable-packer.exe',
                  './hamrahdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/hamrahdesk_portable.exe')
    os.rename('./hamrahdesk_portable.exe', f'./hamrahdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/hamrahdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/hamrahdesk.exe target/release/HamrahDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\hamrahdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/HamrahDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/hamrahdesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/hamrahdesk-{version}-win7-install.exe ../..')
